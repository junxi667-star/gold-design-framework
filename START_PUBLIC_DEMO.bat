...
findstr /B /C:"DEMO_ACCESS_CODE=" "%~dp0.env" | findstr /V /R "DEMO_ACCESS_CODE=$" >nul
if errorlevel 1 (
...
call "%~dp0START_JEWELCHAIN.bat"
if errorlevel 1 exit /b 1
set "APP_PORT=4173"
if exist "%~dp0.gold-demo-server.json" (
  for /f "usebackq delims=" %%P in (`powershell -NoProfile -ExecutionPolicy Bypass -Command "try { $s=Get-Content -Raw '%~dp0.gold-demo-server.json' | ConvertFrom-Json; Write-Output $s.port } catch { exit 1 }"`) do set "APP_PORT=%%P"
)
echo.
echo ====================================================
...
echo ====================================================
echo.
"%~dp0cloudflared.exe" tunnel --url "http://127.0.0.1:%APP_PORT%"
echo.
